...
behavior HoareMutex
    ...
    --         on the mutex.
    --    giveALock ()
    --         make sure lock will be passed to the next thread after signal
	...
	oldIntStat = SetInterruptsTo (DISABLED)
       ...
       -----------Hoare  Mutex . IsHeldByCurrentThread  -----------
	...
	--------------------------Hoare  giveALocK ----------------------------
	method giveALock (t : ptr to Thread)
	var
	  oldIntStat: int
	  oldIntStat = SetInterruptsTo (DISABLED)
          if ! self.IsHeldByCurrentThread ()
            FatalError ("Attempt to give a lock ")
          endIf
	  heldBy = t
	  oldIntStat = SetInterruptsTo (oldIntStat)
	endMethod
  ...
  behavior ThreadManager
      ...
      method GetANewThread () returns ptr to Thread
        ...
        var
          ...
          if (freeList.IsEmpty())                             -- check free status,if thread not available, wait for a thread to be available
            while( ! currThread )
              ...
              if (! currThread)
                aThreadBecameFree.Wait(&threadManagerLock)    -- wait on condition to thread becomes free
              endIf
            endWhile
          else
            currThread = freeList.Remove()                    -- freeList is not empty
          endIf
      ...
      method FreeThread (th: ptr to Thread)
        -- 
        -- This method is passed a ptr to a Thread;It sets the Threads status
        -- as UNUSED and It moves it to the FREE list. 
        -- 
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        -- initialize freeList and place all PCB on freeList

        freeList = new List[ProcessControlBlock]
        
        -- initialize the processTable array

        processTable = new array of ProcessControlBlock { MAX_NUMBER_OF_PROCESSES of new ProcessControlBlock }

        -- initialize all ProcessControlBlocks in processTable array
	...
	/*  processTable[0].Init()
	  ...
	  processTable[i].Init()
        ...
        -- initialize processManagerLock
        processManagerLock = new Mutex
        processManagerLock.Init()

        -- initialize aProcessBecameFree and aProcessDied condition variables
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        -- until one is available.
        --
          var current_Process: ptr to ProcessControlBlock

        -- Remove the PCB from the freeList
          processManagerLock.Lock()
	--  check free status,if PCB not available, wait for a PCB to be available
          if (freeList.IsEmpty())
            while (! current_Process)                        
              if (processManagerLock.IsHeldByCurrentThread())
                current_Process = freeList.Remove()
              endIf
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- to the FREE list.and mark its status as FREE
        --
        processManagerLock.Lock()
        p.status = FREE
        freeList.AddToEnd(p)
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
         var 
         	...
         	-- Acquire the lock
         	frameManagerLock.Lock ()
		numOfWaitingFrames = numOfWaitingFrames + 1
	-- If other groups are waiting in line then wait at the end of a line
		if numOfWaitingFrames > 1 
			restFrameGroup.Wait(&frameManagerLock)                 
		endIf
	...
	numOfWaitingFrames = numOfWaitingFrames - 1
	restFrameGroup.Signal(&frameManagerLock)
...
--
-- This method create a user-level process which runs in its own address space
-- While running this method the cpu will be in "user mode"
-- 
	...
	-- Invoke a Fork to run a Thread. This thread should execute StartUserPeocess function
	-- Supply zero as an argument.
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
	...
	--free the frames in the previous address space
...
-----------------------------  Handle_Sys_Create  ---------------------------------
 ...
 function Handle_Sys_Create (filename: ptr to array of char) returns int
      -- NOT IMPLEMENTED
			...
			printPtr("virtual addr of filename",filename)
			print("filename = ")
			print(&strBuffer)
			nl()
			return 4000

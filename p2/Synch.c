  ...
  behavior Mutex
	...
	--	heldBy: ptr to Thread
		if heldBy == currentThread
      	 		FatalError ("Thread already holding the mutex lock")
		endIf
		oldIntStat = SetInterruptsTo (DISABLED)          -- disable interrupt
		if !heldBy
			heldBy = currentThread
		else 
            		waitingThreads.AddToEnd (currentThread)           -- add to the end of waiting thread list
            		currentThread.Sleep ()
		endIf
		oldIntStat = SetInterruptsTo(oldIntStat)           -- enable interrupt
	...
	--	heldBy: ptr to Thread
		t: ptr to Thread
		if heldBy != currentThread
      	 		FatalError ("Thread not holding the mutex lock")
		endIf
		oldIntStat = SetInterruptsTo (DISABLED)          -- interrupt disabled 
		t = waitingThreads.Remove ()                   
		if t
			t.status = READY
			readyList.AddToEnd (t)                      -- If the threads are waiting then add them to end of ready list
			heldBy = t
		else 
			heldBy = null
		endIf
		oldIntStat = SetInterruptsTo(oldIntStat)                   -- interrupt enabled 
      endMethod
      ...
      method IsHeldByCurrentThread () returns bool
          ...
          --FatalError ("Unimplemented method")
          --return false
	  return heldBy == currentThread                          -- check weather lock is held by current thread. 
      endMethod

...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
	...
	-- Initialize a each new Thread
	print("thread initialization started. \n")  
	...
	-- set each thread status to UNUSED
	...
	-- Innitialize condition variable
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- 
          var current_Thread : ptr to Thread
	  threadManagerLock.Lock()
	--
	-- check freeList contains a thread or not
	-- If not, wait on condition variable
	--
	if (freeList.IsEmpty())
		while(!current_Thread)
			if (threadManagerLock.IsHeldByCurrentThread())
				current_Thread = freeList.Remove()
			endIf
		...
		current_Thread = freeList.Remove()                           -- FreeList is not empty
	endIf
	
	current_Thread.status = JUST_CREATED 

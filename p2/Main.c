...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    -- new variables 
    buffer_plock: Mutex = new Mutex     -- mutex lock to producer 
    buffer_clock: Mutex = new Mutex     -- mutex lock to consumer
    buffer_print_lock: Mutex = new Mutex    -- mutex lock to print current condition 
    buffer_full: Semaphore = new Semaphore       
    buffer_empty: Semaphore = new Semaphore

  function ProducerConsumer ()

      buffer_plock.Init()
      buffer_clock.Init()
      buffer_print_lock.Init()
      buffer_full.Init(0)
      buffer_empty.Init(BUFFER_SIZE)	
  ...
  function Producer (myId: int)
	...
	buffer_empty.Down()          -- decrement the semaphore count
	buffer_plock.Lock()          -- producer lock acquired 

        buffer_print_lock.Lock()
        -- Add c to the buffer
	...
	PrintBuffer (c)
        buffer_print_lock.Unlock()

        -- Perform synchronization...

	buffer_full.Up()                     -- increment the semaphore count
	buffer_plock.Unlock()                -- producer lock released
  ...
  function Consumer (myId: int)
	...
	buffer_full.Down()    -- decrement semaphore count
	buffer_clock.Lock()      -- consumer lock acquired 
	...
	buffer_empty.Up()                     -- increment semaphore count
	buffer_clock.Unlock()                 -- consumer lock released
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  function PhilosphizeAndEat (p: int)
    -- The parameter "p" identifies whiHch philosopher this is.
  ...
  class ForkMonitor
    ...
    fields
      ...
      signal_eating: array[5] of Condition        -- condition variables to moniters to execute wait() and signal()
      moniterLock: Mutex                          -- Moniter's mutex , acquired at the start and released at the end.
  ...
  behavior ForkMonitor
	...
	signal_eating = new array of Condition {5 of new Condition}
	for i = 0 to 4
		signal_eating[i].Init()
	endFor
	...
	self.Test(p)
	if status [p] != EATING 
		signal_eating [p].Wait(& moniterLock)        --  
	endIf
    ...
    method Test (p:int)  -- this method checks self condition also adjusent it are not eating so that it starts eating
	if status[p] == HUNGRY && status[(p+1)%5]!= EATING && status[(p-1)%5] != EATING
		status[p] = EATING
		self.PrintAllStatus()
		signal_eating[p].Signal(& moniterLock)  
	endIf
    endMethod
